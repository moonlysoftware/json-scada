    ...
    setInterval(async function () {
        if (
            ...
            if (metrics.length > 0) {
                ...
                Log.log(
                    "Sparkplug - Publish metrics updates: " + cnt,
                    Log.levelNormal
                );
            }
        } else {
        ...
        if (clientMongo === null)
            ...
            await MongoClient.connect(
                ...
                .then(async (client) => {
                    ...
                    rtCollection = db.collection(
                        jsConfig.RealtimeDataCollectionName
                    );
                    cmdCollection = db.collection(
                    ...
                    if ("autoCreateTags" in connection) {
                        AutoCreateTags = connection.autoCreateTags
                            ? true
                            : false;
                    }
                    ...
                    if (connection.commandsEnabled) {
                        ...
                        const changeStreamCmd = cmdCollection.watch(
                            csCmdPipeline,
                            {
                                fullDocument: "updateLookup",
                            }
                        ...
                        try {
                            ...
                            changeStreamCmd.on("change", (change) => {
                                ...
                                let data = getMetricCommandPayload(
                                    change.fullDocument
                                );
                                if (!data) return;
...
async function getDeviceBirthPayload(
    ...
    res.forEach((element) => {
        ...
        if (
            ...
            if (element.group1 && element.group1.trim() !== "")
                topicName +=
                    AppDefs.TAGS_SUBTOPIC +
                    "/" +
                    topicStr(element.group1) +
                    "/";
    ...
    if (clientMongo && collection && MongoStatus.HintMongoIsConnected)
        while (!ValuesQueue.isEmpty()) {
            try {
                ...
                if (AutoCreateTags) {
                    let topicSplit =
                        data.protocolSourceObjectAddress.split("/");
                ...
                let updTag = {
                    ...
                    causeOfTransmissionAtSource:
                        data?.causeOfTransmissionAtSource,
        ...
        try {
            ...
            spClient.handle.on("reconnect", function () {
                ...
                if (
                    ...
                    Log.log(
                        logMod + "Too many retries will try another broker..."
                    );
            ...
            spClient.handle.on(
                ...
                async function (topic, payload, packet) {
                    ...
                    if (jscadaConnection?.topicsAsFiles instanceof Array)
                        await jscadaConnection.topicsAsFiles.forEach(
                            async (tp) => {
                                if (topicMatchSub(topic)(tp)) {
                                    Log.log(
                                        logMod +
                                            "Received topic as file " +
                                            topic
                                    );
                                    match = true;
                                    try {
                                        // save as file on Mongodb Gridfs
                                        let gfs = new GridFSBucket(
                                            sparkplugProcess.db
                                        );
              ...
              */
                                    ...
                                    if (elem.script) {
                                        ...
                                        try {
                                            ...
                                            if (
                                                sharedObj?.dataArray instanceof
                                                Array
                                            )
                                                sharedObj.dataArray.forEach(
                                                    (element) => {
                                                        if (
                                                            !element.id ||
                                                            !(
                                                        ...
                                                        ValuesQueue.enqueue({
                                                            ...
                                                            transient:
                                                                ...
                                                                true
                                                                    ...
                                                                    : false,
                                                            causeOfTransmissionAtSource:
                                                                "causeOfTransmissionAtSource" in
                                                                element
                    ...
                    if (jscadaConnection?.topics instanceof Array)
                        jscadaConnection.topics.forEach((elem) => {
                            if (elem) {
                                ...
                                if (
                                    jpt.jsonPath !== "" &&
                                    topicMatchSub(topic)(jpt.topic)
                                ) {
                                    // extract value from payload using JSON PATH
                                    let JsonPayload =
                                        TryPayloadAsRJson(payload);
                        ...
                        switch (metric?.name) {
                            case "Node Control/Rebirth":
                                ...
                                if (metric?.value === true) {
                                    Log.log(
                                        logModS +
                                            "Node Rebirth command received"
                                    );
                                    // Publish Node BIRTH certificate
                                    let nbc = getNodeBirthPayload(configObj);
                                    spClient.handle.publishNodeBirth(nbc, {
                                        compress:
                                            AppDefs.SPARKPLUG_COMPRESS_NBIRTH,
                                    });
                                    Log.log(
                                        logMod +
                                            "Publish node birth with " +
                                    ...
                                    spClient.handle.publishDeviceBirth(
                                        ...
                                        {
                                            compress:
                                                AppDefs.SPARKPLUG_COMPRESS_DBIRTH,
                                        }
                                    );
                                }
                                break;
                            case "Node Control/Reboot":
                                ...
                                // if (!jscadaConnection.scadaHostId || jscadaConnection.scadaHostId == '')
                                {
                                    Log.log(
                                        logModS +
                                            "Node Reboot command received, exiting process..."
                                    );
                                    process.exit(999);
                                }
                                break;
                            case "Node Control/Next Server":
                                ...
                                //if (!jscadaConnection.scadaHostId || jscadaConnection.scadaHostId == '')
                                {
                                    Log.log(
                                        logMod +
                                            "Node command Next Server received"
                                    );
                                    if (
                                        jscadaConnection.endpointURLs.length > 1
                                    ) {
                                        ...
                                        Log.log(
                                            logMod +
                                                "No alternatives servers configured!"
                                        );
                                }
                                break;
                        }
            ...
            spClient.handle.on("dcmd", function (deviceId, payload) {
                ...
                if (mongoClient === null || !MongoStatus.HintMongoIsConnected)
                    return;
                Log.log(
                    logModS +
                        "Received DCMD - " +
            ...
            spClient.handle.on("message", function (topic, payload, topicInfo) {
                ...
                Log.log(
                    logModS + "Event: Sparkplug B message on topic: " + topic
                );

                if (Log.levelCurrent >= Log.levelDetailed) {
                    //Log.log(logModS + JSON.stringify(topicInfo), Log.levelDetailed)
                    Log.log(
                        logModS + JSON.stringify(payload),
                        Log.levelDetailed
                    );
                }
...
function queueMetric(metric, deviceLocator, isBirth, templateName) {
    ...
    if ("name" in metric && metric.name.trim() !== "") {
        ...
        if ("alias" in metric) {
            ...
            device.mapAliasToObjectAddress["a" + alias.toString()] =
                objectAddress;
        }
    ...
    if (isBirth) {
        ...
        if ("properties" in metric) {
            if ("description" in metric.properties) {
                catalogProperties.description =
                    metric.properties.description.value;
            }
            ...
            if ("engUnit" in metric.properties)
                catalogProperties.unit =
                    metric.properties.engUnit?.value || "units";
        }
...
async function ProcessDeviceCommand(
    ...
    res.forEach(async (element) => {
        ...
        let cmd = {
            ...
            protocolSourceCommandDuration:
                element.protocolSourceCommandDuration,
